...
def create_slideshow_video(
    ...
    intro_clip = ImageClip(intro_img).set_duration(
        AudioFileClip(intro_audio).duration
    ).set_audio(AudioFileClip(intro_audio))
    ...
    for i, slide in enumerate(content_slides):
        ...
        try:
            ...
            img_clip = ImageClip(slide_img_path).set_duration(duration).set_audio(audio_clip)
    ...
    outro_clip = ImageClip(outro_img).set_duration(
        max(outro_audio_clip.duration, 5.0)
    ).set_audio(outro_audio_clip)
